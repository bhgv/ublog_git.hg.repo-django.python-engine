...
from my_django.test.simple import build_suite, DjangoTestSuiteRunner
from my_django.utils import unittest

def run_tests(*args, **kwargs):
    from my_django.test.simple import run_tests as base_run_tests
...
def geo_apps(namespace=True, runtests=False):
    ...
    `my_django.contrib.gis.tests` that can be used with the current
    database and the spatial libraries that are installed.
    """
    from my_django.db import connection
    from my_django.contrib.gis.geos import GEOS_PREPARE
    ...
    if HAS_GDAL:
        ...
        if connection.ops.postgis and GEOS_PREPARE:
            apps.append('geo3d')
    if runtests:
        return [('my_django.contrib.gis.tests', app) for app in apps]
    elif namespace:
        return ['my_django.contrib.gis.tests.%s' % app
                for app in apps]
    else:
        return apps
...
def geodjango_suite(apps=True):
    ...
    """
    import sys
    from my_django.db.models import get_app

    suite = unittest.TestSuite()

    # Adding the GEOS tests.
    from my_django.contrib.gis.geos import tests as geos_tests
    suite.addTest(geos_tests.suite())

    # Adding the measurment tests.
    from my_django.contrib.gis.tests import test_measure
    ...
    from my_django.contrib.gis.gdal import HAS_GDAL
    if HAS_GDAL:
        from my_django.contrib.gis.gdal import tests as gdal_tests
        suite.addTest(gdal_tests.suite())

        from my_django.contrib.gis.tests import test_spatialrefsys, test_geoforms
    ...
    from my_django.contrib.gis.geoip import HAS_GEOIP
    if HAS_GEOIP and hasattr(settings, 'GEOIP_PATH'):
        from my_django.contrib.gis.geoip import tests as geoip_tests
...
class GeoDjangoTestSuiteRunner(DjangoTestSuiteRunner):
    ...
    def setup_test_environment(self, **kwargs):
        ...
        new_installed =  [
            ...
            'my_django.contrib.sitemaps',
            'my_django.contrib.gis',
        ]
