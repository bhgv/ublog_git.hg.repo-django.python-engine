...
from my_django.contrib.messages import constants, utils, get_level, set_level
from my_django.contrib.messages.api import MessageFailure
from my_django.contrib.messages.storage import default_storage, base
from my_django.contrib.messages.storage.base import Message
from my_django.core.urlresolvers import reverse
from my_django.test import TestCase
from my_django.test.utils import override_settings
from my_django.utils.translation import ugettext_lazy
from my_django.utils.unittest import skipIf


def skipUnlessAuthIsInstalled(func):
    return skipIf(
        'my_django.contrib.auth' not in settings.INSTALLED_APPS,
        "my_django.contrib.auth isn't installed")(func)
...
class override_settings_tags(override_settings):
     def enable(self):
        ...
        # my_django.contrib.messages.storage.base module, so after changing
...
class BaseTest(TestCase):
    ...
    urls = 'my_django.contrib.messages.tests.urls'
    levels = {
    ...
    def test_full_request_response_cycle(self):
        ...
        for level in ('debug', 'info', 'success', 'warning', 'error'):
            add_url = reverse('my_django.contrib.messages.tests.urls.add',
                              args=(level,))
    ...
    def test_with_template_response(self):
        ...
        show_url = reverse('my_django.contrib.messages.tests.urls.show_template_response')
        for level in self.levels.keys():
            add_url = reverse('my_django.contrib.messages.tests.urls.add_template_response',
                              args=(level,))
            response = self.client.post(add_url, data, follow=True)
            self.assertRedirects(response, show_url)
            self.assertTrue('messages' in response.context)
    ...
    @override_settings(
        INSTALLED_APPS=filter(
            lambda app:app!='my_django.contrib.messages', settings.INSTALLED_APPS),
