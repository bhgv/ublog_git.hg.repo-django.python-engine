...
from my_django.utils.safestring import mark_safe, SafeData
...
def translation(language):
    ...
    def _fetch(lang, fallback=None):
        ...
        def _translation(path):
            try:
                t = gettext_module.translation('my_django', path, [loc], DjangoTranslation)
                t.set_language(lang)
                return t
            except IOError:
                return None
...
def check_for_language(lang_code):
    ...
    for path in all_locale_paths():
        if gettext_module.find('my_django', path, [to_locale(lang_code)]) is not None:
            return True
    return False
    ...
    for accept_lang, unused in parse_accept_lang_header(accept):
        ...
        for lang, dirname in ((accept_lang, normalized),
            ...
            if lang.lower() not in supported:
                continue
            for path in all_locale_paths():
                if os.path.exists(os.path.join(path, dirname, 'LC_MESSAGES', 'my_django.mo')):
...
def templatize(src, origin=None):
    ...
    """
    from my_django.template import (Lexer, TOKEN_TEXT, TOKEN_VAR, TOKEN_BLOCK,
