...
from my_django.utils import baseconv, simplejson
from my_django.utils.crypto import constant_time_compare, salted_hmac
...
def get_cookie_signer(salt='my_django.core.signing.get_cookie_signer'):
    ...
    try:
        ...
        raise ImproperlyConfigured(
            'Error importing cookie signer %s: "%s"' % (modpath, e))
    return Signer('my_django.http.cookies' + settings.SECRET_KEY, salt=salt)
...
def dumps(obj, key=None, salt='my_django.core.signing', serializer=JSONSerializer, compress=False):
    """
...
def loads(s, key=None, salt='my_django.core.signing', serializer=JSONSerializer, max_age=None):
    """
