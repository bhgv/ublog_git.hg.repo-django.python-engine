...
import my_django
from my_django.core.management.base import CommandError, NoArgsCommand
from my_django.utils.text import get_text_list
from my_django.utils.jslex import prepare_js_for_gettext
...
def copy_plural_forms(msgs, locale, domain, verbosity, stdout=sys.stdout):
    ...
    """
    django_dir = os.path.normpath(os.path.join(os.path.dirname(my_django.__file__)))
    if domain == 'djangojs':
        domains = ('djangojs', 'my_django')
    else:
        domains = ('my_django',)
...
def process_file(file, dirpath, potfile, domain, verbosity,
    ...
    from my_django.utils.translation import templatize
    ...
    elif domain == 'my_django' and (file_ext == '.py' or file_ext in extensions):
        thefile = file
        orig_file = os.path.join(dirpath, file)
...
def make_messages(locale=None, domain='my_django', verbosity=1, all=False,
    ...
    if domain not in ('my_django', 'djangojs'):
        raise CommandError("currently makemessages only supports domains 'my_django' and 'djangojs'")
...
class Command(NoArgsCommand):
    option_list = NoArgsCommand.option_list + (
        ...
        make_option('--domain', '-d', default='my_django', dest='domain',
            help='The domain of the message files (default: "my_django").'),
