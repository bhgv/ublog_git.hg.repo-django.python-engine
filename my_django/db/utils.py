...
def load_backend(backend_name):
    ...
    try:
        ...
        try:
            ...
            available_backends = []
        full_notation = backend_name.startswith('my_django.db.backends.')
        if full_notation:
            backend_name = backend_name[19:] # See #15621.
        if backend_name not in available_backends:
            backend_reprs = map(repr, sorted(available_backends))
            error_msg = ("%r isn't an available database backend.\n"
                         "Try using my_django.db.backends.XXX, where XXX "
                         "is one of:\n    %s\nError was: %s" %
            ...
            error_msg = ("%r isn't an available database backend.\n"
                         "Try using my_django.db.backends.%s instead.\n"
                         "Error was: %s" %
...
class ConnectionHandler(object):
    ...
    def ensure_defaults(self, alias):
        ...
        conn.setdefault('ENGINE', 'my_django.db.backends.dummy')
        if conn['ENGINE'] == 'my_django.db.backends.' or not conn['ENGINE']:
            conn['ENGINE'] = 'my_django.db.backends.dummy'
