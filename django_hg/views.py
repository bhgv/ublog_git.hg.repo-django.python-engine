        ...
        while c > end :
            ...
            for f in ctx.files():
                if re.search(q, f):
                    ...
                    files.append(f)
            if (re.search(q, ctx.description() )  #codecs.decode(ctx.description(), 'utf8'))
            #if (codecs.decode(ctx.description(), 'utf8').lower().find(q.lower()) > -1
                or re.search(q, codecs.decode(ctx.user(), 'utf8'))
            #    or (codecs.decode(ctx.user(), 'utf8').lower().find(q.lower()) > -1)
                or re.search(q, str(ctx))
            #    or (str(ctx).find(q) > -1)
                or (len(files) > 0)):
...
def repo(request, name, action, rev, path='', display = None):
    ...
    if request.GET.get('cmd') and protocol.iscmd( request.GET.get('cmd') ):   # in HgRepository.cmds:
        ...
        try :
            ...
            resp = protocol.call( r, req, request.GET.get('cmd') )  #(r, req)
