...
from my_dulwich.tests import skipIf
from my_dulwich.tests.utils import (
    open_repo,
    tear_down_repo,
...
class GitClientTests(TestCase):
    ...
    def test_caps(self):
        agent_cap = (
            'agent=dulwich/%d.%d.%d' % my_dulwich.__version__).encode('ascii')
...
class SubprocessSSHVendorTests(TestCase):
    ...
    def setUp(self):
        ...
        self._orig_popen = my_dulwich.client.subprocess.Popen
        my_dulwich.client.subprocess.Popen = DummyPopen

    def tearDown(self):
        my_dulwich.client.subprocess.Popen = self._orig_popen
